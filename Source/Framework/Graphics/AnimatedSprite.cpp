...
namespace GameDev2D
{
    AnimatedSprite::AnimatedSprite(const std::string& aFilename) : SpriteAtlas(aFilename),
        ...
        m_DoesLoop(false),
		m_Pause(false),
        m_DrawLastFrameOnCompletion(true)
    {
    ...
    void AnimatedSprite::Draw()
    {
        ...
        if (m_DoesLoop == false && GetFrameIndex() == GetFrameCount()-1 && m_DrawLastFrameOnCompletion == false)
        {
            shouldDraw = false;
        }
    ...
    bool AnimatedSprite::DoesDrawLastFrameOnCompletion()
    {
        return m_DrawLastFrameOnCompletion;
    }

    void AnimatedSprite::SetDrawLastFrameOnCompletion(bool drawLastFrameOnCompletion)
    {
        m_DrawLastFrameOnCompletion = drawLastFrameOnCompletion;
    }
